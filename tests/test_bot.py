...
from helpers.luis_helper import predict_to_book_details
...
# This class will mainly test if cities are well detected as entities
class TestLuisApp(unittest.TestCase):
    ...
    # Test here if intent are well detected
    def test_utilisation_and_creation_book_details(self):
        ...
        # Check if the booking flight intent has been detected
        self.assertEqual(intent,Intent.BOOK_FLIGHT.value)
        
        # Check if Berlin and Dublin have been detected as entities
...
class NumberPromptTest(aiounittest.AsyncTestCase):
    async def test_number_prompt(self):
        async def exec_test(turn_context:TurnContext):
            ...
            if (results.status == DialogTurnStatus.Empty):
                options = PromptOptions(
                    prompt=MessageFactory.text("what could be your budget for this trip ?")
                )
        ...
        step1 = await adapter.test('Hello', 'what could be your budget for this trip ?')
        step2 = await step1.send('About 2000 bucks.')
        await step2.assert_reply("2000")
